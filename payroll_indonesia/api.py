...
def get_salary_slips_by_employee(employee, year=None):
    ...
    try:
        ...
        if year:
            year = cint(year)
            filters.update({"start_date": [">=", f"{year}-01-01"], "end_date": ["<=", f"{year}-12-31"]})
        ...
        salary_slips = frappe.get_all(
            ...
            fields=[
                ...
                "gross_pay", "net_pay", "total_deduction", "posting_date",
                "company", "month", "is_using_ter", "ter_rate", "total_working_days",
                "total_bpjs", "npwp", "ktp"
            ],
        ...
        for slip in salary_slips:
            ...
            slip["formatted_gross_pay"] = frappe.format(slip["gross_pay"], {"fieldtype": "Currency"})
            slip["formatted_net_pay"] = frappe.format(slip["net_pay"], {"fieldtype": "Currency"})
            slip["formatted_total_deduction"] = frappe.format(slip["total_deduction"], {"fieldtype": "Currency"})
            ...
            slip["month_name"] = month_names[slip_month-1] if 1 <= slip_month <= 12 else f"Month {slip_month}"
        ...
        for y in results:
            results[y]["formatted_total_gross"] = frappe.format(results[y]["total_gross"], {"fieldtype": "Currency"})
            results[y]["formatted_total_net"] = frappe.format(results[y]["total_net"], {"fieldtype": "Currency"})
        ...
        frappe.log_error(f"Error getting salary slips for {employee}: {str(e)}\n{frappe.get_traceback()}", "API Error")
...
def get_salary_slip(name):
    ...
    try:
        ...
        result = {
            ...
            "formatted_total_deduction": frappe.format(doc.total_deduction, {"fieldtype": "Currency"}),
            "company": doc.company,
        ...
        frappe.log_error(f"Error getting salary slip {name}: {str(e)}\n{frappe.get_traceback()}", "API Error")
...
def get_recent_salary_slips(limit=10, company=None):
    ...
    try:
        ...
        salary_slips = frappe.get_all(
            ...
            fields=[
                "name", "employee", "employee_name", "start_date", "end_date", 
                "posting_date", "gross_pay", "net_pay", "company"
            ],
        ...
        for slip in salary_slips:
            slip["formatted_gross_pay"] = frappe.format(slip["gross_pay"], {"fieldtype": "Currency"})
            slip["formatted_net_pay"] = frappe.format(slip["net_pay"], {"fieldtype": "Currency"})
            slip["period"] = f"{frappe.format(slip['start_date'], {'fieldtype': 'Date'})} to {frappe.format(slip['end_date'], {'fieldtype': 'Date'})}"
