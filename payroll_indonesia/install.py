...
def setup_payroll_components():
    ...
    components = [
        ...
        {"name": "PPh 21", "type": "Deduction", "abbr": "PPH", "variable_based_on_taxable_salary": 1}
    ]
...
def migrate_from_json_to_doctype():
    ...
    try:
        ...
        settings_exists = frappe.db.exists("Payroll Indonesia Settings", "Payroll Indonesia Settings")
        if settings_exists:
            settings = frappe.get_doc("Payroll Indonesia Settings", "Payroll Indonesia Settings")
            if settings.app_updated_by and settings.app_updated_by == "dannyaudian" and settings.app_version == "1.0.0":
                logger.info("Settings already migrated, skipping")
                return True
        
        # Get path to defaults.json
        config_path = frappe.get_app_path("payroll_indonesia", "payroll_indonesia", "config", "defaults.json")
        ...
        settings.include_holidays_in_total_working_days = defaults.get("include_holidays_in_total_working_days", 0)
        ...
        frappe.db.commit()
        logger.info("Successfully migrated settings from defaults.json to Payroll Indonesia Settings")
...
def migrate_ter_rates(ter_rates):
    ...
    try:
        ...
        if existing_entries > 10:  # Arbitrary threshold to check if migration is needed
            logger.info(f"Found {existing_entries} existing TER entries, skipping TER rates migration")
            return True
