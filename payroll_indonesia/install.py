...
def migrate_from_json_to_doctype():
    ...
    try:
        ...
        settings.app_last_updated = app_info.get("last_updated", getdate().strftime("%Y-%m-%d %H:%M:%S"))
        ...
        frappe.db.commit()
        frappe.msgprint("Successfully migrated settings from defaults.json to Payroll Indonesia Settings DocType")
...
def migrate_ter_rates(ter_rates):
    ...
    try:
        ...
        if existing_entries > 0:
            frappe.msgprint(f"Found {existing_entries} existing TER entries, skipping TER rates migration")
            return
        ...
        for category, rates in ter_rates.items():
            for rate_data in rates:
                ...
                child_doc.description = f"{category} {rate_data.get('income_from', 0):,.0f}" + \
                                       (f" - {rate_data.get('income_to', 0):,.0f}" if rate_data.get("income_to", 0) > 0 else " onwards")
