    ...
    try:
        ...
        if slip.docstatus != 1:
            ...
            raise frappe.exceptions.ValidationError(
                _("Salary Slip {0} must be submitted (current status: {1})").format(
                    salary_slip, 
                    "Draft" if slip.docstatus == 0 else "Cancelled" if slip.docstatus == 2 else "Unknown"
                )
            )
        ...
        for field in required_fields:
            if not hasattr(slip, field) or not getattr(slip, field):
                ...
                raise frappe.exceptions.ValidationError(
                    _("Salary Slip {0} is missing required field: {1}").format(
                        salary_slip, field
                    )
                )
...
def validate_tax_related_fields(slip: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if hasattr(slip, "employee"):
            result["status_pajak"] = frappe.db.get_value("Employee", slip.employee, "status_pajak") or ""
