...
def validate(doc, method):
    ...
    try:
            ...
            if status and len(status) >= 2:
                try:
                    ...
                    if status_tanggungan != tanggungan:
                        ...
                        frappe.msgprint(_("Jumlah tanggungan disesuaikan dengan status pajak."), indicator="blue")
                except (ValueError, IndexError):
                    # Non-critical error - we can continue with existing value
                    frappe.log_error(
                        "Invalid status_pajak format: {0}. Expected format like TK0 or K1.".format(status),
        ...
        frappe.log_error(
            "Error validating Employee {0}: {1}".format(doc.name if hasattr(doc, 'name') else '', str(e)),
...
def create_custom_fields():
    ...
    try:
        ...
        frappe.msgprint(
            _("Warning: Could not create all custom fields for Indonesian Payroll. See error log for details."),
            indicator="orange"
        )