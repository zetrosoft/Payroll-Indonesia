...
class CustomPayrollEntry(PayrollEntry):
    def validate(self):
        ...
        try:
            ...
            if hasattr(self, 'employees') and not self.employees:
                frappe.msgprint(
                    _("Tidak ada karyawan yang ditemukan. Memeriksa filter..."),
                    indicator="blue"
                )
                    ...
                    if not active_employees:
                        ...
                        if not employees_with_structure:
                            ...
                            for emp in employees_with_structure:
                                ...
                                if existing_slip:
                                    frappe.msgprint(
                                        _("Karyawan {0} sudah memiliki slip gaji untuk periode ini: {1}").format(
                                            emp.employee_name, existing_slip
                                        ),
                                        indicator="blue"
                                    )
    ...
    def _validate_payroll_dates(self):
        ...
        try:
            ...
            if start_month != end_month:
                frappe.throw(
                    _("Untuk perhitungan pajak Indonesia, periode payroll harus berada dalam bulan yang sama"),
        ...
        try:
            if hasattr(self, 'employees') and self.employees:
                ...
                if invalid_employees:
                    ...
                    frappe.msgprint(
                        _("Ditemukan {0} data karyawan yang tidak valid. Data ini akan diabaikan saat pemrosesan.").format(
            ...
            frappe.msgprint(
                _("Error validating employee list. Some employees may be skipped. See error log for details."),
                indicator="orange"
            )
    ...
    def get_emp_list(self):
        ...
        try:
            ...
            frappe.log_error(
                "Employee count: {0}".format(len(minimal_emp_list)),
                "Employee Query"
            )
            ...
            if filtered_emp_list:
                ...
                frappe.msgprint(
                    _("Karyawan yang akan diproses: {0}").format(employee_names),
                    indicator="blue"
                )
            else:
                frappe.msgprint(
    ...
    def create_salary_slips(self):
        ...
        try:
            ...
            # Log aktifitas
            frappe.msgprint(
                _("Membuat slip gaji untuk {0} karyawan").format(len(employees)),
                indicator="blue"
            )
        ...
        try:
            ...
            for emp in employees:
                ...
                if not employee or not frappe.db.exists("Employee", employee):
                    ...
                    frappe.msgprint(
                        _("Employee {0} tidak valid, melewati pembuatan slip gaji").format(employee or "None"),
                        indicator="orange"
                    )
                    continue
                ...
                try:
                    ...
                    if publish_progress:
                        ...
                        unique_employees = {e.get('employee') for e in employees if e.get('employee')}
            ...
            if error_count > 0:
                frappe.msgprint(
                    _("Created {0} salary slips with {1} errors. See error log for details.").format(count, error_count),
                    indicator="orange"
                )
            else:
                frappe.msgprint(
                    _("Successfully created {0} salary slips.").format(count),
                    indicator="green"
                )
    ...
    def get_salary_slip_args(self, employee):
        ...
        try:
            ...
            if not employee:
                frappe.throw(
                    _("Employee ID tidak boleh kosong"),
            ...
            if not employee_doc.date_of_joining:
                frappe.throw(
                    _("Please set the Date Of Joining for employee <strong>{0}</strong>").format(employee),
            ...
            frappe.throw(_("Error generating salary slip data for {0}: {1}").format(employee, str(e)))