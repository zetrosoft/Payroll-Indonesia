...
def calculate_monthly_pph_with_ter(doc, employee):
    ...
    try:
        ...
        total_earnings = (
            flt(sum(flt(e.amount) for e in doc.earnings))
        ...
        is_annual, reason, monthly_gross_pay = detect_annual_income(
            doc.gross_pay, total_earnings, basic_salary, bypass_detection
        )
...
def verify_calculation_integrity(
    ...
    try:
        ...
        if abs(flt(doc.gross_pay) - original_values["gross_pay"]) > 0.01:
            errors.append(f"gross_pay changed: {original_values['gross_pay']} → {doc.gross_pay}")
        ...
        if errors:
            ...
            frappe.msgprint(
                _(
                    "Some TER calculation values were automatically corrected. See the log for details."
                ),
                indicator="orange",
            )
        ...
        frappe.msgprint(
            _("Warning: Could not verify TER calculation integrity"), indicator="orange"
        )
        return False
...
def get_ytd_totals_from_tax_summary_legacy(employee, year, month):
    ...
    try:
        ...
        result = {"ytd_gross": ytd_gross, "ytd_tax": ytd_tax, "ytd_bpjs": ytd_bpjs}
