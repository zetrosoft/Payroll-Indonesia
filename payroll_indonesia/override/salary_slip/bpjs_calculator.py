...
def calculate_bpjs_components(doc, employee, base_salary):
    ...
    try:
        ...
        if not bpjs_config:
            ...
            frappe.log_error(
                "Employee {0} not enrolled in any BPJS program - skipping calculation".format(
                    employee_info
                ),
        ...
        if bpjs_values["total_employee"] <= 0:
            # Non-critical information - log but continue
            frappe.log_error(
                "No BPJS contributions calculated for {0}. Check BPJS settings.".format(
                    employee_info
                ),
        ...
        frappe.log_error(
            "Updating BPJS components in salary slip {0}".format(
        ...
        if bpjs_values["kesehatan_employee"] > 0:
            update_component_amount(
                doc, "BPJS Kesehatan Employee", bpjs_values["kesehatan_employee"], "deductions"
            )

        # BPJS JHT Employee
        if bpjs_values["jht_employee"] > 0:
            update_component_amount(
                doc, "BPJS JHT Employee", bpjs_values["jht_employee"], "deductions"
            )

        # BPJS JP Employee
        if bpjs_values["jp_employee"] > 0:
            update_component_amount(
                doc, "BPJS JP Employee", bpjs_values["jp_employee"], "deductions"
            )
...
def verify_bpjs_components(doc):
    ...
    try:
        ...
        if not hasattr(doc, "deductions") or not doc.deductions:
            frappe.log_error(
                "No deductions found in doc {0}".format(
