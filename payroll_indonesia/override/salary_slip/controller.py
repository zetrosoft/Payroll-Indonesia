    ...
    def set_component(self, component_name, amount, is_deduction=False):
        ...
        try:
            ...
            if not found:
                ...
                target.append({"salary_component": component_name, "amount": amount})
        except Exception as e:
...
def setup_fiscal_year_if_missing(date_str=None):
    ...
    try:
        ...
        if fiscal_year:
            return {"status": "exists", "fiscal_year": fiscal_year}
        ...
        if (
            ...
            == "payroll_indonesia.override.salary_slip.controller.setup_fiscal_year_if_missing"
        ):
            frappe.throw(
...
def process_salary_slips_batch(salary_slips=None, slip_ids=None, batch_size=50):
    ...
    try:
        ...
        if not slip_ids:
            frappe.throw(
                _("No salary slips provided for batch processing"), title=_("Missing Input")
            )
        ...
        for i in range(0, len(slip_ids), batch_size):
            ...
            for slip_id in batch_ids:
                try:
                        ...
                        batch_results["slip_results"].append(
                            {
                                "slip": slip_id,
                                "status": "skipped",
                                "message": "Salary slip not in draft status (docstatus={0})".format(
                                    slip.docstatus
                                ),
                            }
                        )
                    ...
                    results["failed"] += 1
                    results["errors"].append({"slip": slip_id, "error": str(e)})

                    batch_results["slip_results"].append(
                        {"slip": slip_id, "status": "error", "message": str(e)}
                    )
            ...
            frappe.db.commit()
            frappe.db.set_value(
                "Background Job Settings", None, {"last_batch_processed_timestamp": now_datetime()}
            )
            frappe.db.commit()
        ...
        frappe.msgprint(
            _(
                "Processed {0} salary slips: {1} successful, {2} failed. See error log for details."
            ).format(results["total"], results["successful"], results["failed"]),
        ...
        frappe.log_error("Error in batch processing: {0}".format(str(e)), "Batch Process - Error")

        # Add error to results, but also throw since this is user-initiated
        results["errors"].append({"global_error": str(e)})

        frappe.throw(
            _("Batch processing failed: {0}").format(str(e)), title=_("Batch Processing Failed")
        )
...
def clear_caches():
    ...
    try:
        ...
        # Log cache clearing
        frappe.log_error("TER rate and YTD tax caches cleared", "Cache Clearing")
        ...
        frappe.log_error("Error clearing caches: {0}".format(str(e)), "Cache Clearing Error")
...
def diagnose_system_resources():
    ...
    try:
        ...
        return {"memory_usage": {"status": "psutil not installed"}}
    except Exception as e:
        # Non-critical error - log and return status
        frappe.log_error(
            "Error diagnosing system resources: {0}".format(str(e)), "Resource Diagnosis Error"
        )
        return {"memory_usage": {"status": "error", "message": str(e)}}
