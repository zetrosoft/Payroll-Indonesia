    ...
    try:
        ...
        if getattr(doc, "is_using_ter", 0):
            ...
            if not getattr(doc, "ter_category", ""):
                frappe.log_error(
                    "Using TER but no category set for {0}".format(doc.name), "TER Warning"
                )
            ...
            if not getattr(doc, "ter_rate", 0):
                frappe.log_error(
                    "Using TER but no rate set for {0}".format(doc.name), "TER Warning"
                )
...
def clear_caches():
    ...
    try:
        ...
        frappe.log_error("Salary slip caches cleared successfully", "Cache Clearing Success")
