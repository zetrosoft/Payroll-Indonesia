    ...
    try:
        ...
        if isinstance(employee_doc, str):
            try:
                ...
                debug_log(
                    f"Error getting employee document for ID {employee_doc}: {str(e)}", trace=True
                )
        ...
        employee_name = (
            getattr(employee_doc, "name", str(employee_doc))
            if not is_dict
            else str(employee_doc.get("name", "unknown"))
        )
        debug_log(
            f"BPJS enrollment status for {employee_name}: Kesehatan={kesehatan_enrolled}, Ketenagakerjaan={ketenagakerjaan_enrolled}"
        )
        ...
        debug_log(
            f"Final enrollment status for {employee_name}: {is_enrolled} with {len(config)} programs"
        )
    ...
    try:
        ...
        if isinstance(employee, str):
            try:
                ...
                debug_log(
                    f"Error getting employee {employee}: {str(e)}", employee=employee, trace=True
                )
                return result
        ...
        if not base_salary or base_salary <= 0:
            debug_log(
                f"Invalid base salary: {base_salary} for employee {employee_info}. Attempting to use gross_pay or default UMR.",
            ...
            if hasattr(emp_doc, "gross_salary") and emp_doc.gross_salary > 0:
                ...
                debug_log(
                    f"Using employee gross salary as base: {base_salary}", employee=employee_info
                )
            else:
                # Use default UMR (Jakarta minimum wage as safe default)
                base_salary = DEFAULT_UMR
                debug_log(
                    f"Using default UMR as base salary: {base_salary}", employee=employee_info
                )
        ...
        if not settings:
            try:
                ...
                debug_log(
                    f"Error getting BPJS Settings: {str(e)}. Using defaults.",
                    employee=employee_info,
                    trace=True,
                )
                settings = DEFAULT_BPJS_RATES
        ...
        if "kesehatan" in config:
            ...
            debug_log(
                f"Calculated BPJS Kesehatan: Employee={result['kesehatan_employee']}, Employer={result['kesehatan_employer']}",
                employee=employee_info,
            )
        ...
        if "jht" in config:
            ...
            debug_log(
                f"Calculated BPJS JHT: Employee={result['jht_employee']}, Employer={result['jht_employer']}",
                employee=employee_info,
            )
        ...
        if "jp" in config:
            ...
            debug_log(
                f"Calculated BPJS JP: Employee={result['jp_employee']}, Employer={result['jp_employer']}",
                employee=employee_info,
            )
        ...
        if "jkk" in config:
            ...
            debug_log(
                f"Calculated BPJS JKK: Employer={result['jkk_employer']}", employee=employee_info
            )
        ...
        if "jkm" in config:
            ...
            debug_log(
                f"Calculated BPJS JKM: Employer={result['jkm_employer']}", employee=employee_info
            )
        ...
        # Log result safely
        debug_log(
            f"BPJS calculation successful. Total employee={result['total_employee']}, total employer={result['total_employer']}",
            employee=employee_info,
        )
        ...
        frappe.log_error(
            f"Error calculating BPJS: {str(e)}\nTraceback: {frappe.get_traceback()}",
...
def get_bpjs_enrollment_status(employee):
    ...
    debug_log(
        f"Employee {employee_name} BPJS enrollment status: {is_enrolled}, enrolled in {len(config) if config else 0} programs"
    )
...
def update_all_bpjs_components():
    ...
    try:
        ...
        frappe.log_error(
            f"Error updating BPJS components: {str(e)}\nTraceback: {frappe.get_traceback()}",
            "BPJS Update Error",
        )
