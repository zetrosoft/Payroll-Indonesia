...
def create_bpjs_summaries():
    ...
    try:
        ...
        for company in companies:
            ...
            existing = frappe.db.exists("BPJS Payment Summary", {
                "company": company,
            ...
            if not existing:
                try:
                    ...
                    frappe.logger().info(f"Created BPJS Payment Summary for {company} - {first_day}")
                except Exception as e:
                    frappe.logger().error(f"Error creating BPJS Payment Summary for {company}: {str(e)}")
                    continue
                    
        frappe.logger().info("Completed monthly BPJS summary creation")
                    
    except Exception as e:
        frappe.log_error(f"Error in monthly BPJS summary creation: {str(e)}", "BPJS Monthly Task Error")