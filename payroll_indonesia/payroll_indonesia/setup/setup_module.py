...
def create_company_mapping(company):
    ...
    try:
        ...
        account_mapping = (
            get_default_config().get("gl_accounts", {}).get("bpjs_account_mapping", {})
        )
        ...
        if not mapping_exists:
            try:
                ...
                if mapping_name:
                    ...
                    debug_log(
                        f"Failed to create BPJS Account Mapping for {company}", "BPJS Setup Error"
                    )
                    return False
            except ImportError:
                frappe.log_error("Could not import create_default_mapping", "BPJS Setup")
    ...
    try:
        ...
        if hasattr(bpjs_settings, "setup_accounts"):
            try:
                ...
                debug_log(
                    f"Error setting up accounts for {company}: {str(e)}",
                    "BPJS Setup Error",
                    trace=True,
                )
                return False
        ...
        debug_log(
            f"Error in setup_company_bpjs for {company}: {str(e)}", "BPJS Setup Error", trace=True
        )
        return False
...
def setup_pph21_ter_categories():
    ...
    try:
        ...
        frappe.log_error(
            f"Error setting up PMK 168/2023 TER categories: {str(e)}", "PPh 21 Setup Error"
        )
        debug_log(
            f"Error setting up PMK 168/2023 TER categories: {str(e)}",
            "PPh 21 Setup Error",
            trace=True,
        )
        return False
    ...
    try:
        ...
        for category in ["TER A", "TER B", "TER C"]:
            if frappe.db.exists("PPh 21 TER Table", {"status_pajak": category}):
                ...
                frappe.db.sql(
                    f"DELETE FROM `tabPPh 21 TER Table` WHERE status_pajak = '{category}'"
                )
        ...
        for category, rates in ter_rates.items():
            ...
            for rate_data in rates:
                try:
                    ...
                    is_highest = (
                        "is_highest_bracket" in rate_data and rate_data["is_highest_bracket"]
                    )
                    ...
                    if rate_data["income_to"] == 0:
                        description = (
                            f"{category_desc} - Penghasilan > Rp{rate_data['income_from']:,.0f}"
                        )
                    elif rate_data["income_from"] == 0:
                        description = (
                            f"{category_desc} - Penghasilan ≤ Rp{rate_data['income_to']:,.0f}"
                        )
                    else:
                    ...
                    frappe.log_error(
                        f"Error creating TER rate {category}: {str(e)}", "PPh 21 Setup Error"
                    )
                    debug_log(
                        f"Error creating TER rate {category}: {str(e)}",
                        "PPh 21 Setup Error",
                        trace=True,
                    )
...
def setup_ptkp_ter_mapping(mapping_data):
    ...
    try:
        ...
        if not frappe.db.exists("DocType", mapping_doctype):
            debug_log(
                f"{mapping_doctype} DocType not found, creating custom mapping", "PPh 21 Setup"
            )
        ...
        for ptkp_status, ter_category in mapping_data.items():
            try:
                ...
                frappe.log_error(
                    f"Error creating mapping for {ptkp_status}: {str(e)}", "PPh 21 Setup Error"
                )
                debug_log(
                    f"Error creating mapping for {ptkp_status}: {str(e)}",
                    "PPh 21 Setup Error",
                    trace=True,
                )
        ...
        debug_log(
            f"Error setting up PTKP to TER mapping: {str(e)}", "PPh 21 Setup Error", trace=True
        )
        return 0
...
def setup_ptkp_ter_mapping_in_settings(mapping_data):
    ...
    try:
        ...
        frappe.db.rollback()
        frappe.log_error(
            f"Error setting up PTKP to TER mapping in settings: {str(e)}", "PPh 21 Setup Error"
        )
        debug_log(
            f"Error setting up PTKP to TER mapping in settings: {str(e)}",
            "PPh 21 Setup Error",
            trace=True,
        )
        return False
...
def update_pph21_settings_notes():
    ...
    try:
        ...
        debug_log(
            f"Error updating PPh 21 Settings notes: {str(e)}", "PPh 21 Setup Error", trace=True
        )
        return False
...
def setup_salary_components():
    ...
    try:
        ...
        for component in deductions:
            ...
            if (
                component.get("name") == "PPh 21"
                and "supports_ter" in component
                and component["supports_ter"]
            ):
...
def create_salary_component(component_config, component_type):
    ...
    try:
        ...
        if "variable_based_on_taxable_salary" in component_config:
            component.variable_based_on_taxable_salary = component_config.get(
        ...
        if (
            name == "PPh 21"
            and "supports_ter" in component_config
            and component_config["supports_ter"]
        ):
            # Check if custom field exists
            if frappe.db.exists(
                "Custom Field", {"dt": "Salary Component", "fieldname": "supports_ter"}
            ):
        ...
        frappe.log_error(
            f"Error creating salary component {component_config.get('name')}: {str(e)}",
            "Payroll Setup",
        )
        debug_log(
            f"Error creating salary component {component_config.get('name')}: {str(e)}",
