...
def map_ptkp_to_ter_category(status_pajak: str) -> str:
    ...
    try:
        if not status_pajak:
            ...
            frappe.log_error(
                "Empty tax status provided for TER category mapping", "TER Category Mapping Error"
            )
        ...
        try:
            ...
            frappe.log_error(
                f"Error retrieving TER mapping from settings: {str(e)}", "TER Mapping Error"
            )
            frappe.msgprint(
        ...
        if status_pajak == "TK0":
            ...
            frappe.msgprint(
                _("Warning: Unknown tax status {0} for TER mapping. Using {1} as default.").format(
                    status_pajak, result
                ),
                indicator="orange",
            )
        ...
        frappe.throw(
            _("Failed to determine TER category for tax status {0}: {1}").format(
                status_pajak, str(e)
            )
        )
...
def get_ter_mapping_from_settings() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Non-critical error - we can continue with default mapping
        frappe.log_error(
            f"Error retrieving TER mapping from PPh 21 Settings: {str(e)}", "TER Settings Error"
        )
...
def get_ter_rate(income: float, category: str = "TER C") -> float:
    ...
    try:
        ...
        if category not in TER_CATEGORIES:
            ...
            frappe.msgprint(
                _("Warning: Invalid TER category '{0}'. Using '{1}' as default.").format(
        ...
        query = """
            ...
            LIMIT 1
        """
        ter = frappe.db.sql(query, [category, income, income], as_dict=1)
        ...
        query = """
            ...
            LIMIT 1
        """
        ter = frappe.db.sql(query, [category], as_dict=1)
        ...
        frappe.msgprint(
            _(
                "Warning: Could not find TER rate for category '{0}' and income {1}. Using default rate {2}%."
            ).format(category, income, default_rate * 100),
            indicator="orange",
        )
