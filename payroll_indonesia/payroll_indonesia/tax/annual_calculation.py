...
def hitung_pph_tahunan(employee, tahun_pajak):
    ...
    try:
        ...
        try:
            ...
            frappe.throw(
                _("Error calculating PTKP: {0}").format(str(e)),
                title=_("PTKP Calculation Failed")
            )
        ...
        frappe.throw(
            _("Error in annual tax calculation: {0}").format(str(e)),
    ...
    try:
        ...
        for key, value in ptkp_settings.items():
            if key.startswith(prefix):
                ...
                frappe.msgprint(
                    _("PTKP status {0} not found, using closest match {1}").format(status_pajak, key),
                    indicator="orange"
                )
                return flt(value)
        ...
        if prefix in default_values:
            ...
            frappe.msgprint(
                _("PTKP status {0} not found in settings. Using default value.").format(status_pajak),
                indicator="orange"
            )
        ...
        frappe.log_error(
            "Could not find PTKP match for {0}, using TK0 default (54,000,000)".format(status_pajak),
            "PTKP Default Used"
        )
        frappe.msgprint(
...
def generate_december_correction_note(calc_result):
    ...
    try:
        ...
        if not calc_result or not isinstance(calc_result, dict):
            frappe.throw(
                _("Invalid calculation result provided"),
        ...
        if tax_details and isinstance(tax_details, list):
            for i, bracket in enumerate(tax_details):
                ...
                note.append(
                    f"Lapisan {i+1}: Rp {from_amount:,.0f} - Rp {to_text} ({rate:.1f}%)"
                )
                note.append(
                    f"  PKP: Rp {taxable:,.0f} x {rate:.1f}% = Rp {tax:,.0f}"
                )
        ...
        if slip_details and isinstance(slip_details, list):
            ...
            if ter_slips:
                ...
                for slip in ter_slips:
                    ...
                    note.append(
                        "- {0}: {1}, PPh 21: Rp {2:,.0f}".format(slip_date, ter_info, tax)
                    )
        ...
        frappe.log_error(
            "Error generating December correction note: {0}".format(str(e)),
            "December Note Error"
        )
