...
def get_bpjs_settings() -> Dict[str, Any]:
    ...
    return {
        ...
        "jkk": {
            "percent": flt(getattr(settings_doc, 'jkk_percent', 0.24))
        },
        "jkm": {
            "percent": flt(getattr(settings_doc, 'jkm_percent', 0.3))
        }
    }
...
def get_ptkp_settings() -> Dict[str, float]:
    ...
    try:
        ...
        frappe.log_error(f"Error retrieving PTKP settings from Payroll Indonesia Settings: {str(e)}", "PTKP Settings Error")
    ...
    try:
        ...
        if hasattr(settings, 'tax_brackets_table') and settings.tax_brackets_table:
            ...
            for row in settings.tax_brackets_table:
                if hasattr(row, 'income_from') and hasattr(row, 'income_to') and hasattr(row, 'tax_rate'):
                    brackets.append({
        ...
        frappe.log_error(f"Error retrieving PPh 21 brackets from Payroll Indonesia Settings: {str(e)}", "PPh 21 Brackets Error")
            ...
            if hasattr(settings, 'get_ter_rate') and callable(settings.get_ter_rate):
                try:
                    ...
                    if rate is not None:
                        ...
                        frappe.cache().set_value(cache_key, decimal_rate, expires_in_sec=CACHE_MEDIUM)
                        return decimal_rate
                except Exception as e:
    ...
    try:
        ...
        if settings:
            ...
            try:
                ...
                debug_log("Error parsing GL accounts data from settings"/