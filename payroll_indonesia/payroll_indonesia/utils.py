...
# Last modified: 2025-05-11 11:11:36 by dannyaudianlanjutkan
...
from payroll_indonesia.constants import (
    ...
    DEFAULT_BPJS_RATES, BIAYA_JABATAN_PERCENT, BIAYA_JABATAN_MAX,
    TER_CATEGORY_A, TER_CATEGORY_B, TER_CATEGORY_C
)

# Import cache utilities
from payroll_indonesia.utilities.cache_utils import get_cached_value, cache_value, clear_cache, memoize_with_ttl
...
def _get_payroll_settings(doctype: str, cache_key: str, defaults: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        frappe.log_error(
            f"Error retrieving {doctype}: {str(e)}",
            f"{doctype} Retrieval Error"
        )
    
    # If we reach here, use defaults
    frappe.log_error(
        f"{doctype} not found or empty. Using defaults.",
        f"{doctype} Fallback"
    )
...
def get_bpjs_settings() -> Dict[str, Any]:
    """
    Get BPJS settings from the BPJS Settings DocType with caching
    ...
    # Get BPJS Settings using helper
    settings_doc = _get_payroll_settings("BPJS Settings", "bpjs_settings", {})
    ...
    return {
        "kesehatan": {
            "employee_percent": flt(settings_doc.get('kesehatan_employee_percent', 1.0)),
            "employer_percent": flt(settings_doc.get('kesehatan_employer_percent', 4.0)),
            "max_salary": flt(settings_doc.get('kesehatan_max_salary', 12000000))
        },
        "jht": {
            "employee_percent": flt(settings_doc.get('jht_employee_percent', 2.0)),
            "employer_percent": flt(settings_doc.get('jht_employer_percent', 3.7))
        },
        "jp": {
            "employee_percent": flt(settings_doc.get('jp_employee_percent', 1.0)),
            "employer_percent": flt(settings_doc.get('jp_employer_percent', 2.0)),
            "max_salary": flt(settings_doc.get('jp_max_salary', 9077600))
        },
        "jkk": {
            "percent": flt(settings_doc.get('jkk_percent', 0.24))
        },
        "jkm": {
            "percent": flt(settings_doc.get('jkm_percent', 0.3))
        }
    }
...
def get_pph21_settings() -> Dict[str, Any]:
    """
    Get PPh 21 settings from the PPh 21 Settings DocType with caching
    ...
    settings_doc = _get_payroll_settings("PPh 21 Settings", "pph21_settings", {})
    
    if not settings_doc:
        return defaults
        
    # Extract relevant fields
    calculation_method = settings_doc.get('calculation_method', 'Progressive')
    use_ter = cint(settings_doc.get('use_ter', 0))
...
def get_ptkp_settings() -> Dict[str, float]:
    """
    Get PTKP settings from PPh 21 Settings DocType with caching
    ...
    try:
        # Get settings using PPh 21 Settings
        ...
        frappe.log_error(f"Error retrieving PTKP settings from DocType: {str(e)}", "PTKP Settings Error")
...
def get_pph21_brackets() -> List[Dict[str, Any]]:
    """
    Get PPh 21 tax brackets from PPh 21 Settings DocType with caching
    ...
    try:
                ...
                if brackets:
                    ...
                    for i, bracket in enumerate(brackets):
                        ...
                        if not all(k in bracket for k in ['income_from', 'income_to', 'tax_rate']):
                            frappe.log_error(
                                f"Invalid bracket format at index {i}: {bracket}",
        ...
        frappe.log_error(f"Error retrieving PPh 21 brackets: {str(e)}", "PPh 21 Brackets Error")
...
def get_spt_month() -> int:
    ...
    try:
        ...
        settings_doc = _get_payroll_settings("Payroll Indonesia Settings", "payroll_indonesia_settings", {})
        spt_month = settings_doc.get('spt_month')
...
def map_ptkp_to_ter(status_pajak):
    ...
    """
    try:
        # Get mapping from PPh 21 Settings
        settings_doc = _get_payroll_settings("PPh 21 Settings", "pph21_settings", {})
        ter_mapping = {}
        
        # Check if settings has the mapping
        if settings_doc.get('ptkp_ter_mapping_table'):
            mapping_list = settings_doc.get('ptkp_ter_mapping_table', [])
            for row in mapping_list:
                if isinstance(row, dict) and 'ptkp_status' in row and 'ter_category' in row:
                    ter_mapping[row['ptkp_status']] = row['ter_category']
        
        # Check if status exists in mapping
        if status_pajak in ter_mapping:
            return ter_mapping[status_pajak]
...
def should_use_ter():
    ...
    try:
        # Get PPh 21 settings
        settings = get_pph21_settings()
        calc_method = settings.get('calculation_method')
        use_ter = settings.get('use_ter')
...
def get_ytd_totals(employee: str, year: int, month: int, include_current: bool = False) -> Dict[str, Any]:
    """
...
def get_ytd_totals_from_tax_summary(employee: str, year: int, month: int, include_current: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for detail in monthly_details:
            ...
            if flt(detail.gross_pay) > 0:
                ...
                monthly_biaya_jabatan = min(flt(detail.gross_pay) * (BIAYA_JABATAN_PERCENT / 100), BIAYA_JABATAN_MAX)
...
def calculate_ytd_from_salary_slips(employee: str, year: int, month: int, include_current: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if ytd_biaya_jabatan == 0 and ytd_gross > 0:
            ...
            # Use constants for calculation
            ytd_biaya_jabatan = min(ytd_gross * (BIAYA_JABATAN_PERCENT / 100), BIAYA_JABATAN_MAX * months_processed)
...
# Account functions
def find_parent_account(company: str, parent_name: str, company_abbr: str, 
                       account_type: str, candidates: List[str] = None) -> Optional[str]:
    """
...
def create_account(company: str, account_name: str, account_type: str, 
                  parent: str, root_type: Optional[str] = None) -> Optional[str]:
    """
...
def create_parent_liability_account(company: str) -> Optional[str]:
    ...
    try:
        ...
        settings = _get_payroll_settings("Payroll Indonesia Settings", "payroll_indonesia_settings", {})
        account_name = settings.get("bpjs_payable_account_name", "BPJS Payable")
...
def create_parent_expense_account(company: str) -> Optional[str]:
    ...
    try:
        ...
        # Get account name from Payroll Indonesia Settings
        settings = _get_payroll_settings("Payroll Indonesia Settings", "payroll_indonesia_settings", {})
        account_name = settings.get("bpjs_expense_account_name", "BPJS Expenses")
...
def retry_bpjs_mapping(companies: List[str]) -> None:
    ...
    try:
        ...
        settings = _get_payroll_settings("Payroll Indonesia Settings", "payroll_indonesia_settings", {})
        account_mapping = settings.get("bpjs_account_mapping", {})
