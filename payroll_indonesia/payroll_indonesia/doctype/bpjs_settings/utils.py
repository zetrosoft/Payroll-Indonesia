...
def sync_with_payroll_settings(bpjs_doc):
    ...
    try:
        ...
        for field in fields_to_update:
            if hasattr(pi_settings, field) and hasattr(bpjs_doc, field) and pi_settings.get(field) != bpjs_doc.get(field):
        ...
        debug_log(f"Error syncing BPJS Settings to Payroll Indonesia Settings: {str(e)}", "BPJS Settings Sync Error", trace=True)