    ...
    def validate_account_type_match(self):
        ...
        try:
            ...
            if not matches and not ("bpjs payable" in account_name_lower):
                frappe.msgprint(
                    _("Warning: The selected account '{0}' may not be appropriate for BPJS {1}. Expected account should contain: {2}")
    ...
    def before_save(self):
        ...
        if not self.reference_number and self.account_type:
            ...
            if parent_doc and hasattr(parent_doc, "month") and hasattr(parent_doc, "year"):
                ...
                self.reference_number = f"BPJS-{self.account_type}-{frappe.utils.today().replace('-', '')}"
        ...
        try:
            ...
            if hasattr(parent_doc, 'employee_details') and parent_doc.employee_details:
                for emp in parent_doc.employee_details:
                    bpjs_totals["JHT"] += (frappe.utils.flt(emp.jht_employee) + 
                                        frappe.utils.flt(emp.jht_employer))
                    bpjs_totals["JP"] += (frappe.utils.flt(emp.jp_employee) + 
                                      frappe.utils.flt(emp.jp_employer))
                    bpjs_totals["Kesehatan"] += (frappe.utils.flt(emp.kesehatan_employee) + 
            ...
            for bpjs_type, mapping_field in type_to_field_map.items():
                ...
                parent_doc.append("account_details", {
