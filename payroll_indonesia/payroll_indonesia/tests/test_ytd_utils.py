...
class TestYTDCalculations(unittest.TestCase):
    ...
    def setup_test_salary_slips(cls):
        ...
        for month, data in cls.test_data.items():
            ...
            slip = frappe.get_doc({
                ...
                "docstatus": 1 if status == "Submitted" else (2 if status == "Cancelled" else 0)
            }).insert(ignore_permissions=True)
    ...
    def test_ytd_taxable_income_calculation(self):
        ...
        ytd_taxable = get_ytd_total_taxable_income(
    ...
    def test_ytd_pph_paid_calculation(self):
        ...
        self.assertEqual(
            flt(ytd_pph, 2),
            flt(expected_pph, 2),
            "YTD PPh paid calculation mismatch"
        )
    ...
    def test_cancelled_slip_exclusion(self):
        ...
        ytd_april = get_ytd_total_taxable_income(
            self.test_employee.name,
            4,
            self.year
        )
    ...
    def test_draft_slip_exclusion(self):
        ...
        ytd_july = get_ytd_total_taxable_income(
            self.test_employee.name,
            7,
            self.year
        )
    ...
    def test_cache_consistency(self):
        ...
        first_call = get_ytd_total_taxable_income(
            self.test_employee.name,
            9,
            self.year
        )
        ...
        second_call = get_ytd_total_taxable_income(
            self.test_employee.name,
            9,
            self.year
        )
        ...
        third_call = get_ytd_total_taxable_income(
            self.test_employee.name,
            9,
            self.year
        )
        ...
        self.assertEqual(
            flt(first_call, 2),
            flt(third_call, 2),
            "Recomputed value mismatch after cache clear"
        )
    ...
    def test_future_month(self):
        ...
        ytd_december = get_ytd_total_taxable_income(
            self.test_employee.name,
            12,
            self.year
        )
        ...
        ytd_september = get_ytd_total_taxable_income(
            self.test_employee.name,
            9,
            self.year
        )
        ...
        self.assertEqual(
            flt(ytd_december, 2),
            flt(ytd_september, 2),
            "Future month handling incorrect"
        )
