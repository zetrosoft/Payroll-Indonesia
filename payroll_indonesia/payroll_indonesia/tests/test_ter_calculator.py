...
class TestTERCalculator(unittest.TestCase):
    ...
    def test_ter_calculation_basic(self):
        ...
        ter_result = calculate_monthly_pph_with_ter(
            self.test_salary_slip,
            self.test_employee
        )
        ...
        self.assertEqual(
            flt(self.test_salary_slip.monthly_tax),
            flt(self.test_salary_slip.gross_pay * 0.05)
        )
    ...
    def test_ter_annual_projection(self):
        ...
        self.assertEqual(
            flt(self.test_salary_slip.annual_taxable_amount),
            flt(expected_annual)
        )
    ...
    def test_ptkp_mapping(self):
        ...
        for status, expected_category in test_cases:
            ...
            self.assertEqual(
                category,
                expected_category,
                f"Failed PTKP mapping for status {status}"
            )
