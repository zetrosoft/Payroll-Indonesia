...
class TestTaxCalculator(unittest.TestCase):
    ...
    def calculate_expected_bpjs(self, gross_pay):
        ...
        kesehatan = min(
            ...
            self.bpjs_settings.kesehatan_max_salary * self.bpjs_settings.kesehatan_employee_percent / 100
        )
    ...
    def test_complete_setup_ter(self):
        ...
        self.assertEqual(
            flt(salary_slip.monthly_taxable_income, 2),
        ...
        self.assertEqual(
            flt(salary_slip.monthly_tax, 2),
            flt(expected_pph21, 2)
        )
    ...
    def test_annual_projection(self):
        ...
        self.assertEqual(
            flt(salary_slip.annual_taxable_amount, 2),
            flt(expected_annual, 2)
        )

    def test_high_income_progressive(self):
        """Test Progressive calculation for high income"""
        employee = self.test_employees["progressive_override"]
        salary_slip = self.create_salary_slip(
            employee,
            gross_pay=50000000  # 50 juta rupiah
        )
